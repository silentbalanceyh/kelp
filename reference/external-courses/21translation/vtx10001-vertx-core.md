...
我们已经知道，Vert.x的API都是非阻塞式的并且不会阻塞事件循环，但是如果您在自己的处理器中阻塞了事件循环，那么这不是很有帮助。

如果这样做，那么该事件循环在被阻塞时就不能做任何事情。如果您阻塞了Vertx实例中的所有事件循环，那么您的应用就会完全停止！
...
如果上述任何一种情况都阻止了事件循环做任何事并占用了**大量可用时间**，那您做了淘气的事，立即进入淘气的一步，等待进一步指示。
